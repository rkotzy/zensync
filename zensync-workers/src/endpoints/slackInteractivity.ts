import {
  ...
  sanitizeZendeskConnectionInputs
} from '@/lib/utils';
...
export class SlackInteractivityHandler {
  async handle(
    ...
    // Handle the setup zendesk modal submission
    else if (
      payload.type === 'view_submission' &&
      payload.view?.callback_id === InteractivityActionId.ZENDESK_SETUP_MODAL_ID
    ) {
      try {
        await saveNewZendeskCredentials(
          payload,
          slackConnectionInfo,
          env,
          db,
    ...
    // Handle updating the zendesk connection
    else if (
      payload.type === 'view_submission' &&
      payload.view?.callback_id ===
        InteractivityActionId.ZENDESK_UPDATE_MODAL_ID
    ) {
      try {
        await updateZendeskCredentials(
          payload,
          slackConnectionInfo,
          env,
          db,
        ...
        posthog.capture({
          distinctId: analyticsDistinctId,
          event: 'zendesk_connection_updated',
          groups: { company: analyticsCompanyId }
        });
...
async function saveNewZendeskCredentials(
  payload: any,
  ...
  const values = sanitizeZendeskConnectionInputs(payload.view?.state.values);

  const zendeskDomain = values?.zendeskDomain;
  const zendeskEmail = values?.zendeskEmail;
  const zendeskKey = values?.zendeskKey;
...
async function updateZendeskCredentials(
  payload: any,
  connection: SlackConnection,
  env: Env,
  db: DrizzleD1Database<typeof schema>,
  ...
  try {
    const zendeskTestResponse = await fetch(
      `https://${zendeskDomain}.zendesk.com/api/v2/users/me.json`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!zendeskTestResponse.ok) {
      // If the response status is not OK, log the status and the response text
      safeLog(
        'error',
        `Updating Zendesk API failed with status: ${zendeskTestResponse.status}`
      );
      safeLog('error', `Response: ${await zendeskTestResponse.text()}`);
      throw new Error('Invalid Zendesk credentials');
    }
  } catch (error) {
    safeLog('error', 'Error updating zendesk credentials', error);
    throw error;
  }

  // If the request is successful, update the credentials in the database
  try {
    const encryptedApiKey = await encryptData(zendeskKey, key);

    await updateZendeskConnection(
      db,
      connection.id,
      zendeskDomain,
      zendeskEmail,
    ...
    const body = JSON.stringify({
      ...
      view: {
        ...
        callback_id: `${
          zendeskInfo
            ? InteractivityActionId.ZENDESK_UPDATE_MODAL_ID
            : InteractivityActionId.ZENDESK_SETUP_MODAL_ID
        }`,
        title: {
          type: 'plain_text',
          ...
          },
          {
            ...
            element: {
              ...
              placeholder: {
                type: 'plain_text',
                text: `${
                  zendeskInfo?.encryptedZendeskApiKey
                    ? '•••••••••••••••••••••••••'
                    : 'API Key'
                }`
              }
